...
public class LAppMinimumModel extends CubismUserModel {
    ...
    public void draw(CubismMatrix44 matrix) {
        // The sample closed the activity here; a service has no window, so a
        // model that failed to load simply draws nothing.
        if (model == null) {
            return;
        }
    ...
    private boolean setupModel(String model3JsonPath) {
        ...
        // loadFileAsBytes returns null for a missing asset, so this has to be
        // checked before it is handed to the JSON parser.
        if (model3Json == null) {
            CubismFramework.coreLogFunction("[ERROR]model3.json could not be read: " + model3JsonPath);
            return false;
        }
        ...
        if (this.modelSetting.getJson() == null) {
            ...
            // setupModel() returns whether the model loaded.
            return false;
        }
