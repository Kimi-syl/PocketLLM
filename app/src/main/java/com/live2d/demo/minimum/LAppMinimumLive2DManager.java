...
public class LAppMinimumLive2DManager {
    ...
    /**
     * Loads a different model, which is how the app switches between the bundled
     * sample characters. Importing user-supplied models is deliberately not
     * supported: that is what would make this an "Expandable Application" under
     * Live2D's publication licence.
     */
    public void loadModelByName(String modelDirectoryName) {
        // The sample's model class exposes no close(); dropping the reference is
        // what the sample does itself when a model is replaced.
        model = null;
        loadModel(modelDirectoryName);
    }
    ...
    private LAppMinimumLive2DManager() {
        // Intentionally empty: the model is chosen by the caller, so the app is
        // not forced to pay for loading a default one it may immediately replace.
    }
